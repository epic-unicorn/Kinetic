...
void main() {
  ...
  group('Proposal round-trip integration tests', () {
    ...
    test('proposal created on A appears in B after both sync', () async {
      ...
      final storedInSharedStorage =
          storage.listChildren('/kinetic/shared/proposals').toList();
      expect(
      ...
      final rawBytesInStorage =
          storage.get('/kinetic/shared/proposals/prop-1.json');
      expect(
        rawBytesInStorage,
        isNotNull,
      ...
      expect(rawBytesInStorage!.isNotEmpty, isTrue,
          reason: 'Bytes in storage are empty');
      final decryptedStoredBytes =
          await KineticEncryption.decrypt(rawBytesInStorage, _familyKey);
      expect(decryptedStoredBytes.isNotEmpty, isTrue,
          reason: 'Stored bytes cannot be decrypted');

      // Verify that B's WebDavClient.get returns the same bytes as storage.
      final clientGetBytes = await serviceB.client
          .get('/kinetic/shared/proposals/prop-1.json');
      expect(clientGetBytes, equals(rawBytesInStorage),
          reason:
      ...
      expect(serviceB.config.familyKeyBytes, equals(_familyKey),
    ...
    test('after A syncs, local proposal is marked clean', () async {
      await _insertDirtyProposal(dbA, id: 'prop-2', fromParentId: 'parent-a', taskTitle: 'Test');
      await orchestratorA.syncWithService(serviceA);
    ...
    test('B accepts proposal and A sees accepted status after next sync', () async {
      ...
      await (dbB.update(dbB.partnerProposals)
            ..where((t) => t.id.equals('prop-accept')))
          .write(PartnerProposalsCompanion(
    ...
    test('B dismisses proposal and A sees dismissed status after sync', () async {
      await _insertDirtyProposal(
        dbA,
        id: 'prop-dismiss',
        fromParentId: 'parent-a',
      ...
      await (dbB.update(dbB.partnerProposals)
            ..where((t) => t.id.equals('prop-dismiss')))
          .write(PartnerProposalsCompanion(
    ...
    test('LWW: later updatedAt wins when both sides update same proposal', () async {
      ...
      await (dbA.update(dbA.partnerProposals)
            ..where((t) => t.id.equals('prop-lww')))
          .write(PartnerProposalsCompanion(
    ...
    test('device with wrong family key cannot read proposals from server', () async {
    ...
    test('syncing multiple times does not duplicate proposals', () async {
      ...
      final rows = await (dbB.select(dbB.partnerProposals)
            ..where((t) => t.id.equals('prop-idem')))
          .get();
  ...
  group('FakeHttpClient and WebDavSyncService low-level sanity', () {
    ...
    test('FakeHttpClient PROPFIND raw body contains hrefs', () async {
      storage.put('/kinetic/shared/proposals/p1.json', Uint8List.fromList([1]));
      storage.put('/kinetic/shared/proposals/p2.json', Uint8List.fromList([2]));
    ...
    test('PROPFIND lists stored files', () async {
      ...
      final storageChildren = storage.listChildren('/kinetic/shared/proposals/').toList();
      expect(
        storageChildren,
        hasLength(2),
      ...
      final entries = await service.client.propfind('/kinetic/shared/proposals');
      expect(
        entries.length,
        greaterThan(0),
        reason:
      ...
      final jsonEntries = entries.where((e) => e.href.endsWith('.json')).toList();
    ...
    test('task category and priority survive the round-trip', () async {
      final now = DateTime.now().toUtc();
      await dbA.into(dbA.partnerProposals).insert(
        PartnerProposalsCompanion.insert(
          id: 'prop-meta',
          fromParentId: 'parent-a',
