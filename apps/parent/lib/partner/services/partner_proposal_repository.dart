...
class PartnerProposalRepository {
  ...
  PartnerProposalRepository({required AppDatabase db, required TodoRepository todoRepository})
      : _db = db,
  ...
  Stream<List<PartnerProposal>> watchPending({String? myParentId}) {
    final proposalsStream = (_db.select(_db.partnerProposals)
    ...
    final tasksStream = (_db.select(_db.personalTasks)
          ..where((t) => t.isCompleted.equals(false) & t.syncState.equals('deleted').not()))
        .watch();

    return proposalsStream
        .asyncExpand((proposals) => tasksStream.map((taskRows) {
              final ownTitles = taskRows
                  .map((r) => _normalizeTitle(r.title))
                  .toSet();
              return proposals
                  .where(
                    (p) => !ownTitles.contains(_normalizeTitle(p.taskTitle)),
                  )
                  .toList();
            }));
  }
  ...
  Future<void> accept(String proposalId) async {
    ...
    final proposalRow = await (_db.select(_db.partnerProposals)
  ...
  Future<void> createManualProposal({
    ...
    await _db.into(_db.partnerProposals).insert(
