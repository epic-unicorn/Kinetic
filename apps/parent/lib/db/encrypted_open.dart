...
///
/// Requires workspace root `hooks.user_defines.sqlite3.source: sqlite3mc`.
/// Existing plaintext `{name}.sqlite` files are migrated once into
/// `{name}.encrypted.sqlite` via `VACUUM INTO` + `PRAGMA rekey`.
QueryExecutor openEncryptedDatabase({
  required String name,
  required Future<Directory> Function() databaseDirectory,
  String secureStorageKey = 'kinetic_db_encryption_key',
}) {
  return LazyDatabase(() async {
    ...
    return NativeDatabase.createInBackground(
      ...
      setup: (rawDb) {
        ...
        // Verify the key / file before Drift starts using the connection.
